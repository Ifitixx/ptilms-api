...
import s3 from './config/aws.js';  // Import S3 client
...
const setupRoutes = (app, container) => {
  ...
  const createBucketIfNotExists = async (bucketName) => {
    try {
      ...
      info(`Bucket "${bucketName}" already exists.`);
      return true;
    } catch (error) {
      if (error.statusCode === 404) {
        ...
        try {
          ...
          _error(`Bucket creation failed: ${createError.message}`);
          throw new Error(`Failed to create bucket: ${createError.message}`);
        }
      }
      _error(`MinIO connection error: ${error.code} - ${error.message}`);
  ...
  const verifyMinioConnection = async () => {
    try {
      info('MinIO Configuration:', JSON.stringify({
        endpoint: config.minio.endpoint,
        accessKey: config.minio.accessKey,
        // Don't log the secret key!
        s3ForcePathStyle: config.minio.s3ForcePathStyle,
        signatureVersion: 'v4',
        region: 'us-east-1',
      }, null, 2));
      const buckets = await s3.listBuckets().promise();
      info('MinIO connection verified. Existing buckets:', buckets.Buckets);
      return true;
    } catch (error) {
      _error('MinIO connection failed:');
  ...
  const initializeServer = async () => {
    try {
      ...
      info('MinIO connection verified.');
  
      // Create MinIO bucket if it doesn't exist
      const bucketName = config.minio.bucketName; // Get bucket name from config
      await createBucketIfNotExists(bucketName);
      info('MinIO bucket check/creation complete.');
